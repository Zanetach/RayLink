...
3. 远程 GeoIP/Geosite 规则集尚未随订阅提供离线回退，首次下载失败时会影响客户端启动；订阅二维码也尚未实现。
...
| TUN、DNS、国内直连、自动选路 | 已实现 | 已通过 1.13.12 语法校验；需多平台实机验收与规则集离线回退 |
...
当前实现使用 SagerNet 官方远程 rule-set、每日更新并启用 sing-box 缓存。正式发布前仍需固定
规则集版本与 SHA-256，并增加首次启动时的离线回退，避免下载源暂时不可达导致客户端启动失败。

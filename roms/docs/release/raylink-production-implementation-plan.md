...
| 用户到期、流量额度、节点范围 | 已实现 | 真实字节计量、跨节点累计和超额撤权已贯通 |
...
| TLS 资产分发 | 已实现 | 节点专属密封、证书密钥校验、`0600` 原子写入和失败恢复 |
...
2. 每个 RayLink Node 本地生成 X25519 密钥对，私钥只保存在节点 `0600` 状态文件。
3. 控制面使用临时 X25519、HKDF-SHA256 和 AES-256-GCM 生成节点专属密封包；任务数据库不保存明文私钥。
4. 配置按主机改写为 `/var/lib/raylink-node/sing-box/tls/releases/<证书指纹>/` 不可变受管路径。
5. Node 执行任务时重新检查证书有效期和密钥匹配，先完整写入新版本资产，再以原子配置切换启用；中途崩溃不会改变旧配置引用的证书对。
...
- 控制面按“主机 + Runtime 实例 + 用户 + 方向”计算增量，持久化精确字节账本，处理重启归零、跨节点汇总、重复采集和离线恢复。
- 达到额度后撤销授权并触发配置发布。
- Runtime 安装和升级都校验 `with_v2ray_api`，避免在线升级后静默丢失计量能力。
- 缺少该 build tag 的主机不会生成估算用量；界面根据真实样本和错误上报显示“等待样本、健康、数据中断、采集故障、能力缺失”。
...
- [x] TLS 私钥不出现在 API、日志、任务结果和普通数据库导出中；任务库只含节点专属密文。
- [ ] 用量采集在 Runtime 重启、节点离线、重复上报和跨节点并发场景完成 72 小时故障验收。

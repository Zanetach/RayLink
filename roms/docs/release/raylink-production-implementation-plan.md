...
但当前仍是 **正式版候选基础，不满足完整正式发布条件**。以下四项是发布阻断项：

1. 客户端配置只有本地 mixed 代理入口和默认代理出口，尚未生成 TUN、DNS 劫持、国内直连和境外代理的完整智能路由。
2. 用户中心提供登录后下载 JSON，尚无稳定的用户订阅地址、二维码、吊销/重置和客户端自动更新链路。
3. TLS 证书文件只保存路径，没有安全分发到远程主机；跨主机发布 Trojan、AnyTLS、Naive 等 TLS 协议可能因缺少证书失败。
4. 当前流量用量由外部写入，没有自动、持久、按用户的计量链路，因此不能承诺流量配额会自动准确扣减。
...
远程 rule-set 应固定版本、校验 SHA-256、每日更新并启用缓存。首次下载失败时使用随客户端配置交付的最后可用版本，不能因为 GitHub Raw 暂时不可达导致客户端完全启动失败。
...
GET /sub/{public-id}/{secret}/metadata
POST /api/users/{id}/subscription/rotate
POST /api/users/{id}/credentials/rotate
```

要求：

- 订阅密钥仅保存哈希，不能明文写入数据库或日志。
- 管理界面展示订阅 URL 和二维码；用户中心可复制、下载和重置。
- 用户停用或到期后返回明确的 403，不继续下发节点凭据。
- 重置订阅链接不必自动重置协议运行凭据；两类操作分别确认。
- 响应支持 ETag、短期缓存和审计，但不得被公共 CDN 缓存。
- 订阅内容只包含当前用户的凭据和授权节点。
...
| B | 正式订阅 URL/二维码/吊销、完整 TUN + DNS + 智能路由生成 | 5–8 人日 |

...
v0.2.19 Release 当前支持 AMD64（x86_64）和 ARM64（aarch64）。服务器需要预先具备 `curl`。
使用 root 登录时，直接复制执行这一条命令：

```bash
bash -o pipefail -c 'curl -fsSL https://github.com/Zanetach/RayLink/releases/download/v0.2.19/install.sh | bash'
```

普通用户登录时，把管道中的 `bash` 改为 `sudo bash`：

```bash
bash -o pipefail -c 'curl -fsSL https://github.com/Zanetach/RayLink/releases/download/v0.2.19/install.sh | sudo bash'
```
...
```bash
bash -o pipefail -c 'curl -fsSL https://github.com/Zanetach/RayLink/releases/download/v0.2.19/install.sh | bash -s -- --public-ip 203.0.113.10'
```

安装指定版本：

```bash
bash -o pipefail -c 'curl -fsSL https://github.com/Zanetach/RayLink/releases/download/v0.2.19/install.sh | bash -s -- --version 0.2.19'
```

只验证下载、校验和解压，不修改系统：

```bash
bash -o pipefail -c 'curl -fsSL https://github.com/Zanetach/RayLink/releases/download/v0.2.19/install.sh | bash -s -- --dry-run'
```
...
本地也可以在 Runtime 产物准备完成后构建单架构正式安装包。v0.2.19 默认装配
AMD64 Runtime：

```bash
bash deploy/package-release.sh 0.2.19
```

也可以显式指定本次发布需要装配的架构：

```bash
RAYLINK_RELEASE_ARCHES=amd64 bash deploy/package-release.sh 0.2.19
```

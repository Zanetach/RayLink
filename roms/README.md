...
- SagerNet 官方 GeoIP/Geosite 规则集自动下载、定期更新与本地缓存
...
远程规则集来自 SagerNet 官方 `sing-geoip`、`sing-geosite` 发布页，由 sing-box 下载并按日更新；
缓存由客户端的 `cache_file` 保存。配置已经通过正式基线 sing-box 1.13.12 和本机
sing-box 1.13.14 的 `sing-box check`。

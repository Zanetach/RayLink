...
test("release metadata publishes a checksummed manifest and SPDX SBOM", async (t) => {
  ...
  const archivePath = join(directory, "raylink-0.2.19-linux-amd64.tar.gz");
  ...
  const manifest = JSON.parse(
    await readFile(join(directory, "raylink-0.2.19-linux-amd64.manifest.json"), "utf8")
  );
  assert.equal(manifest.schemaVersion, 1);
  assert.equal(manifest.version, "0.2.19");
  assert.equal(manifest.architecture, "amd64");
  assert.equal(manifest.archive.filename, "raylink-0.2.19-linux-amd64.tar.gz");
  ...
  const sbom = JSON.parse(
    await readFile(join(directory, "raylink-0.2.19-linux-amd64.spdx.json"), "utf8")
  );
  assert.equal(sbom.spdxVersion, "SPDX-2.3");
  assert.ok(sbom.packages.some((entry) => (
    entry.name === "RayLink" && entry.versionInfo === "0.2.19"
  )));
  assert.ok(sbom.packages.some((entry) => (

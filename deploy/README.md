...
v0.2.26 Release 当前支持 AMD64（x86_64）和 ARM64（aarch64）。服务器需要预先具备 `curl`。
...
```bash
bash -o pipefail -c 'curl -fsSL https://github.com/Zanetach/RayLink/releases/download/v0.2.26/install.sh | bash -s -- --version 0.2.26'
```

只验证下载、校验和解压，不修改系统：

```bash
bash -o pipefail -c 'curl -fsSL https://github.com/Zanetach/RayLink/releases/download/v0.2.26/install.sh | bash -s -- --dry-run'
```
...
本地也可以在 Runtime 产物准备完成后构建单架构正式安装包。v0.2.26 默认装配
AMD64 Runtime：

```bash
bash deploy/package-release.sh 0.2.26
```

也可以显式指定本次发布需要装配的架构：

```bash
RAYLINK_RELEASE_ARCHES=amd64 bash deploy/package-release.sh 0.2.26
```
